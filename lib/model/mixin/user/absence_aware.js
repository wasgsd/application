...
module.exports = function(sequelize){
  ...
  this.get_automatic_adjustment = function(args) {
    ...
    var adjustment = -1*(this.department.allowence - Math.round(
      this.department.allowence * end_date.diff(start_date, 'days') / 365
    ));

    // Take into consideration allowance braough into current year
    // from previous years
    if ( _.isArray( this.carried_over_allowances ) ) {
      var carried_allowance_row = _.find(
        this.carried_over_allowances,
        function(r){ return r.year === now.format('YYYY') }
      );

      if ( carried_allowance_row ) {
        adjustment += carried_allowance_row.transferred;
      }
    }
