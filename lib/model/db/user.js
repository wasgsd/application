  ...
  return {
    ...
    calculate_number_of_days_available_in_allowence : function(year){
      ...
      return this.calculate_total_number_of_days_n_allowence(year)
        - this.calculate_number_of_days_taken_from_allowence({year : year});
    },
    ...
    reload_with_leave_details : function(args){
      ...
      return Promise.join(
        ...
        self.getDepartment(),
        self.getCarried_over_allowances({
          where : {
            year : moment(year).format('YYYY'),
          }
        }),
        function(leaves, department, carried_over_allowances){

          self.my_leaves               = leaves;
          self.department              = department;
          self.carried_over_allowances = carried_over_allowances;
...
function withAssociations() {
  ...
  this.associate = function(models){
    ...
    models.User.hasMany(models.CarriedOverAllowance, {
      as : 'carried_over_allowances',
      foreignKey : 'user_id',
    });
  };
}
