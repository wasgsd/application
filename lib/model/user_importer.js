...
const add_user_interface_schema = Joi.object().required().keys({
    ...
    calendar_id : Joi.number().default(null).allow(null).integer().positive(),
...
function add_user(args) {
  ...
  attributes.CalendarId = args.calendar_id;
...
function add_users_in_bulk(args) {
  ...
  let company,
    ...
    calendar_vector_index = 4,
    startdate_vector_index = 5; // TODO calendar & start date bulk import
