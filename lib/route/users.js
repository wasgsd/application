...
router.post('/add/', function(req, res){
  ...
  .then(() => UserImporter.add_user({
    ...
    calendar_id : new_user_attributes.CalendarId,
      ...
      include : [
        {
          ...
          include : [
            ...
            { 
              model : model.Company, as : 'calendar_override',
...
function promise_user_list_data_for_rendering(args) {
  ...
  let users_info_for_rendering = users_info.map(ui => ({
    ...
    calendar_name                       :  ui.user_row.calendar_override ? ui.user_row.calendar_override.name : null,
...
function get_and_validate_user_parameters(args) {
    ...
    var name     = validator.trim(req.param('name')),
        ...
        calendar_id     = validator.trim(req.param('calendar')),
  ...
  console.log('>>>>>: ' + email);
    ...
    if (!validator.isNumeric(calendar_id)) {
        req.session.flash_error(
            'New calendar number of '+item_name+' should be a valid number'
        );
    }
